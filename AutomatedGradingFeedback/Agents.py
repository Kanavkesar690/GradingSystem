...
EduEvaluator = AssistantAgent(
    ...
    model_client=client(),
    # system_message="""
    #       You are an Assignment Evaluation Agent designed to help teachers evaluate multiple student assignments at once.
    #       Your role is to grade assignments, detect plagiarism, compare with study material, analyze sentiment, generate feedback, and produce class-level analytics.
    ...
    #           - "grade" → numeric value between 0–100 (dynamically calculated based on accuracy, completeness, and relevance).
    #           - "explanation" → short reasoning for the grade.
    #           - "plagiarism" → with "score" (0–100, numeric, dynamically estimated by overlap with study materials or other sources) and "explanation".
    #           - "feedback" → with "strengths", "weaknesses", and "suggestions" (dynamic text).
    #           - "sentiment_analysis" → analysis of assignment tone compared to study material:
    ...
    #           - "average_grade" → average of all student grades.
    #           - "average_plagiarism" → average of all plagiarism scores.
    #           - "most_difficult_question" → the topic/question most students struggled with.
    #           - "common_knowledge_gaps" → list of concepts that appeared weak across many students.
    #           - "grade_distribution" → counts of students in grade ranges "90-100", "75-89", "60-74", "<60".
    #           - "student_clusters" → group students into "high_performers", "average", and "at_risk" based on grades.
    #           - "plagiarism_sources" → summarize plagiarism origins: "course_notes", "internet", "peer_assignments".
    #           - "sentiment_summary" → class-level sentiment trends:
    #             - "positive_count", "neutral_count", "negative_count".
    #             - "confidence_distribution" → counts of "high", "medium", "low".
    ...
    #           - "class_summary"
    #           - "students" (list of per-student evaluations).
